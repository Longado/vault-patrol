...
## How we built it
Python 3.12, FastAPI on Cloud Run (webhook returns 202, patrol runs in background with CPU always allocated). Gemini 3.5 through the Google GenAI SDK with `response_schema` = a pydantic model whose first field is `reasoning`. Every report is stamped with the model version and prompt version, so drift in either is traceable; re-running updates the existing PR rather than opening a second one. GitHub REST for branch + PR. The prompt is a versioned file in `prompts/`, not a string in code.
...
**Built with:** Gemini 3.5, Google GenAI SDK, Cloud Run, Secret Manager, FastAPI, Python, GitHub API
